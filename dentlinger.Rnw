...
A total of \Sexpr{Ntot} (754) stranded green turtles were reported from 1983--2022: \Sexpr{CoastCount$East} \orig{378} stranded on the east (windward) coast of \Hawaii\ Island and \Sexpr{CoastCount$West} \orig{376} on the west (leeward) coast.
Strandings peaked in 2011 and 2018 and were highest from March to August. 
The most common known cause of stranding was hook-and-line fishing gear (\Sexpr{CauseCount$p[2]} \orig{21.4\%} of total strandings), followed by fibropapillomatosis (\Sexpr{CauseCount$p[3]} \orig{7.2\%}), human take (\Sexpr{CauseCount$p[4]} \orig{4.3\%}), miscellaneous (\Sexpr{CauseCount$p[5]} \orig{3.7\%}), boat impact (\Sexpr{CauseCount$p[6]} \orig{3.3\%}), shark attack (\Sexpr{CauseCount$p[7]} \orig{3.2\%}), and net (\Sexpr{CauseCount$p[8]} \orig{2.1\%}); however, \Sexpr{CauseCount$p[1]} \orig{54.8\%} of strandings had no known cause.
...
Across both sides of the island, strandings were less frequent in winter, November--February, than in other months (Table \ref{tab:monthside}). The highest totals were observed between March and August.
...
Of the \Sexpr{Ntot} \orig{754} stranded turtles in the records, slightly over half had no known cause that could be determined (the ``unknown'' cause).
...
The distribution of causes is significantly different between the East and West sides of the island (Chi-square test, $X_7=70$, $p < 10^{-10}$), with the effect being driven most strongly by the FP and Miscellaneous categories (Table \ref{tab:cause}).
...
<<causetab>>=
BigIslandStrandings %>% count(Cause,Coast) %>% pivot_wider(names_from = Coast, values_from = n) %>%
  mutate(Total=East+West) %>% mutate(across(-Cause, . %>% proportions %>% percent(0.1, suffix=""), .names="p_{.col}")) %>%
  relocate(East, p_East, West, p_West, Total, p_Total, .after = Cause) %>%
...
As shown in Table \ref{tab:tumor_side}, \Sexpr{sum(tumor_side_table$N)} \orig{460} records indicated the absence of FP tumors, \Sexpr{sum(tumor_side_table$Y)} \orig{150} records presence of a tumor, and \Sexpr{sum(tumor_side_table$U)} \orig{144} records are missing this observation. Note that the presence of a FP tumor does not necessarily mean that the primary cause of stranding was recorded as FP. 
...
Of all the stranded turtles, \Sexpr{sum(side_status_table$A)} \orig{359 (47.6\%)} stranded alive, \Sexpr{sum(side_status_table$D)} \orig{381 (50.5\%)} stranded dead, and \Sexpr{sum(side_status_table$U)} \orig{14 (1.9\%)} turtles had no stranding status reported.
Stranding status was found to be significantly associated with cause (Chi-square test, $X_{\Sexpr{cause_status_test$parameter}} = \Sexpr{number(cause_status_test$statistic)} \orig{102}$, $p < 10^{-10}$). 
...
Stranding status was also found to be significantly associated with stranding location (Chi-square test, $X_{\Sexpr{side_status_test$parameter}} = \Sexpr{signif(side_status_test$statistic, 3)} \orig{23.2}$, $p = \Sexpr{side_status_test$p.value %>% signif(2)} \orig{9.2 \times 10^{-6}}$).
West \Hawaii\ had \Sexpr{side_status_table["West","A"]} \orig{146 (39.7\%)} turtles strand alive and \Sexpr{side_status_table["West","D"]} \orig{222 (60.3\%)} strand dead, while east \Hawaii\ had \Sexpr{side_status_table["East","A"]} \orig{213 (57.3\%)} turtles strand alive and \Sexpr{side_status_table["East", "D"]} \orig{159 (42.7\%)} strand dead. 
