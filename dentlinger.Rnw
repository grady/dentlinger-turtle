...
\miscnote{Note to publisher: The Hawaiian letter \okina okina is typeset in this manuscript as an open quote, e.g. \Hawaii. In electronic publication, it should be rendered as unicode U+02BB MODIFIER LETTER TURNED COMMA.}
...
<<map, fig.width=3.31, fig.height=3.31>>=
library(tmap)
library(sf)
