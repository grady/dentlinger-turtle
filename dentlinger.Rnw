...
A total of \Sexpr{Ntot} \xorig{754} stranded green turtles were reported from 1983--2022: 
\Sexpr{CoastCount$East} \orig{378} stranded on the east (windward) coast of \Hawaii\ Island and 
\Sexpr{CoastCount$West} \orig{376} on the west (leeward) coast.
Strandings peaked in 2011 and 2018 and were highest from March to August. 
The most common known cause of stranding was 
hook-and-line fishing gear (\Sexpr{CauseCount$p[2]} \xorig{21.4\%} of total strandings), 
followed by fibropapillomatosis (\Sexpr{CauseCount$p[3]} \xorig{7.2\%}), 
human take (\Sexpr{CauseCount$p[4]} \orig{4.3\%}), 
miscellaneous (\Sexpr{CauseCount$p[5]} \xorig{3.7\%}), 
boat impact (\Sexpr{CauseCount$p[6]} \xorig{3.3\%}), 
shark attack (\Sexpr{CauseCount$p[7]} \xorig{3.2\%}), 
and net (\Sexpr{CauseCount$p[8]} \xorig{2.1\%});
however, \Sexpr{CauseCount$p[1]} \xorig{54.8\%} of strandings had no known cause.
...
A total of \Sexpr{Ntot} \xorig{754} green turtles stranded on \Hawaii\ Island from June 1983 to June 2022. 
Of those strandings, 
\Sexpr{CoastCount$West} (\Sexpr{percent(CoastCount$West/Ntot, 0.1, suffix="\\%")}) \orig{376 (49.9\%)}
were located on the leeward or west coast of \Hawaii\ Island, while 
\Sexpr{CoastCount$East} (\Sexpr{percent(CoastCount$East/Ntot, 0.1, suffix="\\%")}) \orig{378 (50.1\%)} 
...
tm_shape(hawaii.sf, simplify=0.0125, bbox=c(-156.15, 18.91, -154.8, 20.27)) + 
  ...
  tm_symbols(shape="Coast", title.shape="Side", alpha=0.2, size=0.3, col="black", border.alpha=1, border.lwd =  1) + 
...
Of the \Sexpr{Ntot} \xorig{754} stranded turtles in the records, slightly over half had no known cause that could be determined (the ``unknown'' cause).
The most common known cause of stranding was hook-and-line fishing gear, accounting for about 1 in 5 strandings.
The distribution of causes is significantly different between the East and West sides of the island (Chi-square test, $X_{\Sexpr{cause_side_test$parameter}}= \Sexpr{signif(cause_side_test$statistic, 3)}  \xorig{70}$, $p < 10^{-10}$), 
...
(BigIslandStrandings %>% count(Year, Side=Coast, name = "Strandings") %>% ggplot(aes(x=Year, y=Strandings, fill=Side, color=Side))) +
  geom_col() + geom_vline(xintercept = 2019.5, lty=2) + 
  scale_fill_grey(start=0.6, end=0.2, labels=c("East Hawaii Island", "West Hawaii Island"), aesthetics = c("color", "fill")) +
...
<<monthside>>=
BigIslandStrandings %>% mutate(Month=month(Date, label=TRUE, abbr=TRUE)) %>% count(Month,Side=Coast) %>%
...
(BigIslandStrandings %>% count(Year, Cause, Side=Coast, name="Strandings") %>% ggplot(aes(x=Year, y=Strandings, fill=Side, color=Side))) +
  geom_col() + 
  facet_wrap(vars(Cause), scales="fixed") + 
  geom_vline(xintercept = 2019.5, lty=2) + 
  scale_fill_grey(start=0.6, end=0.2, labels=c("East Hawaii Island", "West Hawaii Island"), aesthetics = c("fill", "color")) +
  labs(y="Number of strandings", x="Year") + 
  theme_bw() + 
...
has an AIC \Sexpr{signif(deg0model$AIC - deg4model$AIC,2)} \xorig{1.2} smaller than the null model. 
...
\Sexpr{signif(scl_table$min,3)} \xorig{19.8} cm to 
\Sexpr{signif(scl_table$max,3)} \xorig{99} cm straight carapace length (SCL), with a mean of 
\Sexpr{signif(scl_table$mean,3)} \xorig{54.8} cm, across 
\Sexpr{size_class["Juvenile","n"]} \xorig{381} juveniles, 
\Sexpr{size_class["Subadult","n"]} \xorig{88} subadults, and 
\Sexpr{size_class["Adult","n"]} \xorig{19} adults.
No SCL measurement was recorded in \Sexpr{Ntot-scl_table$n} \xorig{266} of the case reports.
Turtles stranding in east \Hawaii\ 
($\mu\pm\text{SE} = \Sexpr{side_scl_table["East", "mean"] %>% signif(3)} 
\pm \Sexpr{side_scl_table["East", "se"] %>% signif(1)}$ cm SCL, $n=\Sexpr{side_scl_table["East", "n"]}$)
...
$F(\Sexpr{size_cause_test$Df})=\Sexpr{size_cause_test["Cause","F value"] %>% signif(3)} \xorig{3.415}$, 
$p=\Sexpr{size_cause_test["Cause","Pr(>F)"] %>% signif(2)} \xorig{0.0014}$), 
the differences between the groups are small compared to the within-group variances.
The records contain 
\Sexpr{gender_table["F"]} \xorig{154 (20.42\%)} female, 
\Sexpr{gender_table["M"]} \xorig{145 (19.23\%)} male, and 
\Sexpr{gender_table["U"]} \xorig{455 (60.35\%)} gender undetermined cases, also with marginally different distributions between side of the island (Chi-square, 
$X_{\Sexpr{gender_side_test$parameter} \xorig{2}}=\Sexpr{signif(gender_side_test$statistic,2)} \orig{6.98}$,
$p=\Sexpr{signif(gender_side_test$p.value, 2)} \orig{0.030}$). 
...
<<tumor_side>>=
tumor_side_table <- BigIslandStrandings %>% count(Side=Coast, Tumor) %>% 
  pivot_wider(names_from = Tumor, values_from = n)

tumor_side_test <- tumor_side_table %>% select(-Side) %>% chisq.test()

tumor_side_table %>% relocate(Side, Present=Y, None=N, "Not Recorded"=U) %>%
  bind_rows(., summarise(., across(where(is.numeric), sum), Side="Total")) %>%
  knitr::kable("latex", booktabs=T, linesep=c("","\\addlinespace")) %>%
...
\Sexpr{sum(tumor_side_table$N)} \xorig{460} records indicated the absence of FP tumors,
\Sexpr{sum(tumor_side_table$Y)} \xorig{150} records presence of a tumor, and
\Sexpr{sum(tumor_side_table$U)} \xorig{144} records are missing this observation. 
Note that the presence of a FP tumor does not necessarily mean that the primary cause of stranding was recorded as FP. 
Tumor presence/absence is significantly associated with side of the island, with turtles stranding in east \Hawaii\ more likely to have tumors than those in west \Hawaii\ (Chi-square test, $X_2=\Sexpr{number(tumor_side_test$statistic)}\orig{198}$, $p < 10^{-10}$).
...
\Sexpr{sum(side_status_table$A)} \xorig{359 (47.6\%)} stranded alive, 
\Sexpr{sum(side_status_table$D)} \xorig{381 (50.5\%)} stranded dead, and 
\Sexpr{sum(side_status_table$U)} \xorig{14 (1.9\%)} turtles had no stranding status reported.
Stranding status was found to be significantly associated with cause 
(Chi-square test, $X_{\Sexpr{cause_status_test$parameter}} = \Sexpr{number(cause_status_test$statistic)} \orig{102}$,
$p < 10^{-10}$).
...
$X_{\Sexpr{side_status_test$parameter}} = \Sexpr{signif(side_status_test$statistic, 3)} \orig{23.2}$, 
$p = \Sexpr{side_status_test$p.value %>% signif(2)} \orig{9.2 \times 10^{-6}}$).
West \Hawaii\ had
\Sexpr{side_status_table["West","A"]} \xorig{146 (39.7\%)} turtles strand alive and
\Sexpr{side_status_table["West","D"]} \orig{222 (60.3\%)} strand dead, 
while east \Hawaii\ had 
\Sexpr{side_status_table["East","A"]} \xorig{213 (57.3\%)} turtles strand alive and
\Sexpr{side_status_table["East", "D"]} \orig{159 (42.7\%)} strand dead. 
...
\Sexpr{str_to_sentence(english(nrow(BigIslandStrandings)))} 
%Seven hundred and fifty-four
green turtles were recorded stranded on \Hawaii\ Island in the period 1983-2022, which represents an unknown fraction of total strandings on Hawaiian shores in that time. 
...
As seen in Figure \ref{fig:sides}, the increase in reported strandings appears to slow in the early 2000s, stabilizing at a rate of approximately 25--30 per year.
This trend was also noticed in studies covering the other main Hawaiian Islands \citep{chaloupka2008cause}. 

There are two years post 2005 which show an unusually large number of Green turtle strandings: 2011 and 2018. The peak in 2011 is associated with the March 2011 magnitude 9.0 T\={o}hoku earthquake off the coast of Japan and the tsunami, large waves, and hazardous currents that it caused around \Hawaii\ Island, and particularly its western shoreline \citep{cheung2013surges}. 
...
Throughout the pandemic, people in general spent much less time in public locations, and for some periods \Hawaii\ County and State beach parks were even closed for recreational use by executive decree \citep{emergencyNo2, emergencyFifth}. 
Similarly, tourism to the island and state were heavily restricted.
...
Strandings of loggerhead, green, and leatherback turtles in Brazil were highest during the austral spring and summer seasons \citep{monteiro2016long}. 
...
Tiger sharks are well-known predators of sea turtles, and green turtles are often found in their stomach contents \citep{witzell1987selective, lowe1996ontogenetic}. 
...
Increased boat presence accompanied with high vessel speeds, varying water depth, and poor water visibility can all factor into a higher proportion of boat impact strandings on the west side of the island \citep{fuentes2021conservation}.
...
Turtles were more likely to have FP on east \Hawaii\ Island, whereas FP was very rare on green turtles that stranded on west \Hawaii\ Island. 
...
Perhaps the greatest factor that could contribute to the absence of FP on west \Hawaii\ Island is the precipitation pattern on the leeward side of the island.
...
The relatively even distribution of turtles that stranded alive (\Sexpr{sum(side_status_table$A)}) \xorig{(359)} versus dead \Sexpr{sum(side_status_table$D)} \xorig{(381)} on \Hawaii\ Island in the present study is markedly different from the findings of \cite{chaloupka2008cause}, where on \Oahu, Maui, and Kauai approximately 75\% of green turtles stranded dead. 
...
The considerable contribution of hook-and-line fishing gear to strandings highlights the continuing need for additional mitigation efforts focused on green turtle interactions with fisheries.
...
Follow-up analyses and modelling by Grady Weyenberg. 
