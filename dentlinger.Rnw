...
Chi-square (X2) goodness of fit tests were used to determine if there were equal proportions among months of stranding, stranding status, causes of stranding, and sex of stranded turtles for all of \Hawaii\ Island. When comparing west and east \Hawaii, contingency tables and chi-square tests of independence were used. All analyses were performed using the statistical software R version 4.2 \citep{rcore2022}. Multinomial linear models were fit using the nnet package \citep{nnet}. Statistical significance was accepted at $p < 0.05$. 
...
\begin{figure}[tb]
PLACEHOLDER
\caption{Stranding locations and the division into eastern and western \Hawaii\ island.}\label{fig:map}
\end{figure}
