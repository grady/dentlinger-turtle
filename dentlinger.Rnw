...
Tumor presence/absence is significantly associated with side of the island, with turtles stranding in east \Hawaii\ more likely to have tumors than those in west \Hawaii\ (Chi-square test, $X_2=198$, $p < 10^{-10}$).
...
tomor_side_test <- tumor_side_table %>% select(-Side) %>% chisq.test()
