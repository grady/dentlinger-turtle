...
BigIslandStrandings <- read.csv("BigIslandStrandings.csv")
...
A total of \Sexpr{nrow(BigIslandStrandings)} (754) stranded green turtles were reported from 1983--2022: \Sexpr{CoastCount$East} (378) stranded on the east (windward) coast of \Hawaii\ Island and \Sexpr{CoastCount$West} (376) on the west (leeward) coast. 
Strandings peaked in 2011 and 2018 and were highest from March to August. 
The most common known cause of stranding was hook-and-line fishing gear (\Sexpr{CauseCount$p[2]} (21.4\%) of total strandings), followed by fibropapillomatosis (\Sexpr{CauseCount$p[3]} (7.2\%)), human take (\Sexpr{CauseCount$p[4]} (4.3\%)), miscellaneous (\Sexpr{CauseCount$p[5]} (3.7\%)), boat impact (\Sexpr{CauseCount$p[6]} (3.3\%)), shark attack (\Sexpr{CauseCount$p[7]} (3.2\%)), and net (\Sexpr{CauseCount$p[8]} (2.1\%)); however, \Sexpr{CauseCount$p[1]} (54.8\%) of strandings had no known cause.
