\documentclass[sn-basic,NameDate,pdflatex]{sn-jnl}
...
\author[1]{\fnm{Skylar} \sur{Dentlinger}}

\author*[2]{\fnm{Karla J.} \sur{McDermid}}%, orcid.org/0000-0002-7663-6545}
%\equalcont{These authors contributed equally to this work.}

\author*[3]{\fnm{Grady} \sur{Weyenberg}}%, orcid.org/0000-0001-6128-1772}
%\equalcont{These authors contributed equally to this work.}

\author[4]{\fnm{Laura M. R.} \sur{Jim}}
\author[5]{\fnm{Marc R.} \sur{Rice}} %, orcid.org/0009-0008-5951-0749}
\author[6]{\fnm{George H.} \sur{Balazs}}

\affil[1]{\orgdiv{Department of Marine Science}, \orgname{University of \Hawaii\ at Hilo},
\orgaddress{\city{Hilo}, \state{Hawaii}, \postcode{96720}, \country{USA}; Current address: \orgdiv{Rosenstiel School of Marine and Atmospheric Science}, \orgname{University of Miami}, \orgaddress{\city{Miami}, \state{Florida}, \postcode{33149}, \country{USA}.}} E-mail: {skylarde@hawaii.edu}}

\affil[2]{\orgdiv{Department of Marine Science}, \orgname{University of \Hawaii\ at Hilo},
\orgaddress{\city{Hilo}, \state{Hawaii}, \postcode{96720}, \country{USA}. 
\href{https://orcid.org/0000-0002-7663-6545}{orcid.org/0000-0002-7663-6545}
$^*$Correspondence: E-mail: {mcdermid@hawaii.edu}}}

\affil[3]{\orgdiv{Department of Mathematics}, \orgname{University of \Hawaii\ at Hilo}, 
\orgaddress{\city{Hilo}, \state{Hawaii}, \postcode{96720}, \country{USA}. 
\href{https://orcid.org/0000-0001-6128-1772}{orcid.org/0000-0001-6128-1772}
$^*$Correspondence: E-mail: {gradysw@hawaii.edu}}}

\affil[4]{\orgdiv{Sea Turtle Research Program}, \orgname{\Hawaii\ Preparatory Academy}, 
\orgaddress{\city{Kamuela}, \state{Hawaii}, \postcode{96743}, \country{USA}. E-mail: {laura.jim@hpa.edu}}}

\affil[5]{\orgdiv{Sea Turtle Research Program}, \orgname{\Hawaii\ Preparatory Academy}, 
\orgaddress{\city{Kamuela}, \state{Hawaii}, \postcode{96743}, \country{USA}. 
\href{https://orcid.org/0009-0008-5951-0749}{orcid.org/0009-0008-5951-0749}
E-mail: {mrice@hpa.edu}}}

\affil[6]{\orgname{Golden Honu Services of Oceania}, 
\orgaddress{\city{Honolulu}, \state{Hawaii}, \country{USA}. E-mail: {itsahonuworld@hotmail.com}}}
...
make_plot_namer <- function(prefix="Figure-", n=1L, width=2){
  function(x){
    ...
    magick::image_write(magick::image_read_pdf(x), path=fs::path_ext_set(x2, "tiff"), density=1200, format="tiff")
    n <<- n + 1L
...
knitr::opts_chunk$set(
  ...
  error = FALSE, ## die on compilation error
...
BigIslandStrandings <- read_csv("BigIslandStrandings.csv") %>% 
  mutate(Cause=case_match(Cause,
                          "boat impact" ~ "Boat impact",
                          "fibropapillomatosis" ~ "FP",
                          "line" ~ "Hook/line",
                          "human take" ~ "Human take",
                          "miscellaneous" ~ "Misc.",
                          "net" ~ "Net",
...
Ntot <- nrow(BigIslandStrandings)
CoastCount <- BigIslandStrandings %>% count(Coast) %>% pivot_wider(names_from = Coast, values_from = n)
CauseCount <- BigIslandStrandings %>% count(Cause) %>% mutate(p=proportions(n) %>% percent(0.1, suffix="\\%")) %>% arrange(desc(n))

@

\abstract{
The Hawaiian population of green turtles (\emph{Chelonia mydas}) has increased since Federal and State protections were implemented in the mid 1970s, and reported stranding events have also increased. 
This study analyzed \Hawaii\ Island data: stranding location, date, size, sex, presence/absence of tumors, stranding status, as well as cause of stranding. 
A total of \Sexpr{Ntot} stranded green turtles were reported from 1983--2022: 
\Sexpr{CoastCount$East} stranded on the east (windward) coast of \Hawaii\ Island and 
\Sexpr{CoastCount$West} on the west (leeward) coast.
Strandings peaked in 2011 and 2018 and were highest from March to August. 
The most common known cause of stranding was 
hook-and-line fishing gear (\Sexpr{CauseCount$p[2]} of total strandings), 
followed by fibropapillomatosis (\Sexpr{CauseCount$p[3]} ), 
human take (\Sexpr{CauseCount$p[4]}), 
miscellaneous (\Sexpr{CauseCount$p[5]}), 
boat impact (\Sexpr{CauseCount$p[6]}), 
shark attack (\Sexpr{CauseCount$p[7]}), 
and net (\Sexpr{CauseCount$p[8]});
however, \Sexpr{CauseCount$p[1]} of strandings had no known cause.
Statistical modeling did not provide convincing evidence of temporal changes in the distribution of strandings across three consolidated cause categories: human-caused; predation, disease, and weather; and unknown.
...
These results provide the first comprehensive analyses of stranding data from \Hawaii\ Island and provide information that can inform resource managers, policy makers, and the public about the various types and magnitudes of impacts, anthropogenic and natural, to green turtles so that mitigation measures can be put into practice.
Our findings allow for comparison with other green turtle populations worldwide.
}

\keywords{sea turtles; fishing gear entanglement; fibropapillomatosis; Hawaiian Islands; marine reptile mortality; Pacific Ocean}
...
The Hawaiian population of green turtles that was once depleted has increased since its 1974 protection under Hawaiian Law and 1978 protection under the Endangered Species Act \citep{balazs2004thirty}. 
...
Hawaiian green turtles have been additionally impacted by hunting at foraging grounds, by harvesting of both eggs and females at nesting grounds, and by the destruction of their nesting habitat. 
...
FP is a debilitating neoplastic disease associated with a herpesvirus found in turtles worldwide \citep{jacobson1991herpesvirus, herbst1994fibropapillomatosis}.
...
An unknown number of deceased turtles never reach shore.
...
Although \cite{chaloupka2008cause} mentioned that 6\% of statewide strandings occurred on \Hawaii\  Island  from 1982--2003, long-term stranding data specifically from \Hawaii\ Island have not been thoroughly analyzed previously.
\Hawaii\ Island merits additional scientific scrutiny of its green turtle stranding patterns with the most up-to-date, inclusive data available  because of the island's large size (over half of total Hawaiian land area), southernmost location in the archipelago, lowest human population density, and important turtle foraging and resting areas---recently proposed as critical habitat by the US Fish and Wildlife Service and the National Oceanic and Atmospheric Administration \citep{fed2023designation, fed2023proposed}.
...
A total of \Sexpr{Ntot} green turtles stranded on \Hawaii\ Island from June 1983 to June 2022. 
Of those strandings, 
\Sexpr{CoastCount$West} (\Sexpr{percent(CoastCount$West/Ntot, 0.1, suffix="\\%")}) 
were located on the leeward or west coast of \Hawaii\ Island, while 
\Sexpr{CoastCount$East} (\Sexpr{percent(CoastCount$East/Ntot, 0.1, suffix="\\%")}) 
...
tm_shape(hawaii.sf, simplify=0.0125, bbox=c(-156.15, 18.91, -154.8, 20.27)) + 
  ...
  tm_shape(BigIslandStrandings %>% 
             ...
             filter(Site != "Mountain View")) + 
  tm_symbols(shape="Coast", title.shape="Coast", alpha=0.2, size=0.3, col="black", border.alpha=1, border.lwd =  1) + 
...
\begin{table}[tbp]
\caption{\color{black}Raw counts and proportions of stranding cause from 1983--2022 for \Hawaii\ island, separated into east and west sides.
...
<<causetab>>=
cause_side_table <- BigIslandStrandings %>% count(Cause,Coast) %>% pivot_wider(names_from = Coast, values_from = n) 
cause_side_test <- cause_side_table %>% select(-Cause) %>% chisq.test()

cause_side_table %>% mutate(Total=East+West, 
                            across(-Cause, . %>% proportions %>% percent(0.1, suffix=""), .names="p_{.col}")) %>%
  relocate(Cause, East, p_East, West, p_West, Total, p_Total) %>%
...
Of the \Sexpr{Ntot} stranded turtles in the records, slightly over half had no discernable cause that could be determined (the ``unknown'' cause).
The most common known cause of stranding was hook-and-line fishing gear, accounting for about 1 in 5 strandings.
The distribution of causes is significantly different between the east and west coasts of the island (Chi-square test, $X_{\Sexpr{cause_side_test$parameter}}= \Sexpr{signif(cause_side_test$statistic, 3)}$, $p < 10^{-10}$), 
with the effect being driven most strongly by the FP and Miscellaneous categories (Table \ref{tab:cause}).

\subsection{Temporal trends}

The number of strandings on \Hawaii\ Island has fluctuated over the years but shows an overall increase over time (Figure \ref{fig:sides}). 
...
(BigIslandStrandings %>% count(Year, Coast, name = "Strandings") %>% ggplot(aes(x=Year, y=Strandings, fill=Coast, color=Coast))) +
  geom_col() + geom_vline(xintercept = 2019.5, lty=2) + 
  scale_fill_grey(start=0.6, end=0.2, labels=c("East Hawaii", "West Hawaii"), aesthetics = c("color", "fill")) +
...
\begin{table}[tbp]
\caption{\color{black}Strandings in each month for \Hawaii\ island, separated into east and west sides.}\label{tab:monthside}
<<monthside>>=
BigIslandStrandings %>% mutate(Month=month(Date, label=TRUE, abbr=TRUE)) %>% count(Month,Coast) %>%
  pivot_wider(names_from = Month, values_from=n) %>% 
  bind_rows(., summarize(., across(where(is.numeric), sum), Coast="Total")) %>%
  rowwise(Coast) %>% mutate(Total=sum(c_across(where(is.numeric)))) %>% 
...
(BigIslandStrandings %>% count(Year, Cause, Coast, name="Strandings") %>% ggplot(aes(x=Year, y=Strandings, fill=Coast, color=Coast))) +
  geom_col() + 
  facet_wrap(vars(Cause), scales="fixed") + 
  geom_vline(xintercept = 2019.5, lty=2) + 
  scale_fill_grey(start=0.6, end=0.2, labels=c("East Hawaii", "West Hawaii"), aesthetics = c("fill", "color")) +
  labs(y="Number of strandings", x="Year") + 
  theme_bw() + 
...
<<modelfit, include=FALSE>>=
threeClass <- BigIslandStrandings %>% 
  mutate(Cause=case_match(Cause,
                          c("Human take", "Hook/line", "Net", "Boat impact") ~ "Human",
                          c("Shark attack", "Misc.", "FP") ~ "Natural",
                          "Unknown" ~ "Unknown") %>% 
           factor(c("Human", "Unknown", "Natural")))

deg3model <- multinom(Cause~ns(Year, df=3), threeClass, Year >= 1985 & Year != 2011, trace=FALSE)
predFrame <- tibble(Year=seq(1983, 2022, by=0.25))

deg3preds <- predict(deg3model, newdata=predFrame, type="probs") %>% bind_cols(predFrame, .)

deg0model <- multinom(Cause~1, threeClass, Year >= 1985 & Year != 2011, trace=FALSE)
deg1model <- multinom(Cause~ns(Year, df=1), threeClass, Year >= 1985 & Year != 2011, trace=FALSE)
deg2model <- multinom(Cause~ns(Year, df=2), threeClass, Year >= 1985 & Year != 2011, trace=FALSE)
deg4model <- multinom(Cause~ns(Year, df=4), threeClass, Year >= 1985 & Year != 2011, trace=FALSE)
@

<<bootstrap, include=FALSE, cache=TRUE>>=
library(purrr)
bootReps <- 2000
clevel <- 0.025

bootReplicates <- rerun(bootReps,
                        multinom(Cause~ns(Year, df=3), 
                                 threeClass %>% slice_sample(prop=1, replace=TRUE), 
                                 Year>=1985 & Year != 2011,
                                 trace=FALSE) %>% 
                          predict(newdata=predFrame, type="probs") %>% 
                          bind_cols(predFrame,.)) %>% bind_rows(.id="run")

bootCI <- bootReplicates %>% group_by(Year) %>% 
  mutate(Human = 1-Human ) %>%
  summarise(across(c(Human, Natural),
                   list(Lower = ~ quantile(.x, clevel),
                        Upper = ~ quantile(.x, 1-clevel)))) %>%
  pivot_longer(-Year, names_to = c("Cause", ".value"), names_sep="_")
@
...
triclassplot <- BigIslandStrandings %>% 
  mutate(Cause=case_match(Cause,
                          c("Human take", "Hook/line", "Net", "Boat impact") ~ "Human",
                          c("Shark attack", "Misc.", "FP") ~ "Natural",
                          "Unknown" ~ "Unknown") %>% 
           factor(c("Human", "Unknown", "Natural"))) %>%
  count(Year, Cause, name="Strandings", .drop = TRUE) %>%
  group_by(Year) %>%
  mutate(Prop=proportions(Strandings)) %>% ggplot(aes(x=Year, y=Prop, color=Cause, fill=Cause))

triclassplot + geom_col() +
  ...
  geom_ribbon(aes(x=Year,ymin=Lower, ymax=Upper, group=Cause), data=bootCI, 
              inherit.aes = FALSE, alpha=0.25, fill="white", color="white") +
  geom_line(aes(x=Year, y=1-Human), deg3preds, color=1, lty=1, inherit.aes = FALSE) +
  geom_line(aes(x=Year, y=Natural), deg3preds, color=1, lty=1, inherit.aes = FALSE) +
...
To reduce the variance of the fitted model parameters, the causes as recorded were consolidated into 3 categories: 
Human-caused (hook-and-line, boat impact, human take, and net); predation, disease and weather (shark attack, FP, and Misc.); and the original unknown category. 
...
The AIC increases going from a null model (AIC \Sexpr{signif(deg0model$AIC,5)}) with no dependence on year
to a predictor function with 2 degrees of freedom (AIC \Sexpr{signif(deg2model$AIC,5)}), 
and then slightly decreases again, so that a 4 degree of freedom model (AIC \Sexpr{signif(deg4model$AIC,5)})
has an AIC \Sexpr{signif(deg0model$AIC - deg4model$AIC,2)} smaller than the null model. 
Figure \ref{fig:model} displays a 3-degree of freedom model (AIC \Sexpr{signif(deg3model$AIC,5)}),
...
<<size_gender, include=FALSE>>=
side_scl_table <- BigIslandStrandings %>% group_by(Coast) %>% filter(!is.na(SCL)) %>% 
  summarize(mean=mean(SCL), sd=sd(SCL), n=n(), se=sd/sqrt(n), min=min(SCL), max=max(SCL)) %>%
  column_to_rownames("Coast")

scl_table <- BigIslandStrandings %>% filter(!is.na(SCL)) %>% 
  summarize(mean=mean(SCL), sd=sd(SCL), n=n(), se=sd/sqrt(n), min=min(SCL), max=max(SCL))

size_class <- BigIslandStrandings %>% filter(!is.na(SCL)) %>% 
  count(Class=case_when(SCL < 65 ~ "Juvenile", 
                        SCL < 81 ~ "Subadult", 
                        SCL >= 81 ~ "Adult")) %>%
  column_to_rownames("Class")

size_cause_test <- aov(SCL~Cause, BigIslandStrandings) %>% summary %>% extract2(1)

gender_table <- BigIslandStrandings %>% count(Sex) %>% column_to_rownames("Sex") %>% t %>% drop

gender_side_table <- BigIslandStrandings %>% count(Sex,Coast) %>% 
  pivot_wider(names_from = Coast, values_from = n) %>% column_to_rownames("Sex")

gender_side_test <- gender_side_table %>% chisq.test()

side_size_test <- t.test(SCL~Coast, BigIslandStrandings)

@


Stranded turtles in the records ranged from 
\Sexpr{signif(scl_table$min,3)} cm to 
\Sexpr{signif(scl_table$max,3)} cm straight carapace length (SCL), with a mean of 
\Sexpr{signif(scl_table$mean,3)} cm, across 
\Sexpr{size_class["Juvenile","n"]} juveniles, 
\Sexpr{size_class["Subadult","n"]} subadults, and 
\Sexpr{size_class["Adult","n"]} adults.
No carapace length measurement was recorded in \Sexpr{Ntot-scl_table$n} of the case reports.
Turtles stranding in east \Hawaii\ 
($\mu\pm\text{SE} = \Sexpr{side_scl_table["East", "mean"] %>% signif(3)} 
\pm \Sexpr{side_scl_table["East", "se"] %>% signif(2)}$ cm SCL, $n=\Sexpr{side_scl_table["East", "n"]}$)
%\orig{($\mu\pm\text{SE}=58.8\pm0.78$ cm SCL, range 19.8--99 cm)} 
were significantly larger
(t-test, $t_{\Sexpr{round(side_size_test$parameter)}}=\Sexpr{signif(side_size_test$statistic,3)}$,
$p=\Sexpr{signif(side_size_test$p.value, 1)}$)
%\orig{(t-test unequal variance, $t_{375}=6.32$, $p=7\times 10^{-10}$)}
than those in west \Hawaii\ 
($\mu\pm\text{SE} = \Sexpr{side_scl_table["West", "mean"] %>% signif(3)} 
\pm \Sexpr{side_scl_table["West", "se"] %>% signif(1)}$ cm SCL, $n=\Sexpr{side_scl_table["West", "n"]}$)
...
$F(\Sexpr{size_cause_test$Df})=\Sexpr{size_cause_test["Cause","F value"] %>% signif(3)}$, 
$p=\Sexpr{size_cause_test["Cause","Pr(>F)"] %>% signif(2)}$), 
the differences between the groups are small compared to the within-group variances.
The records contain 
\Sexpr{gender_table["F"]} female, 
\Sexpr{gender_table["M"]} male, and 
\Sexpr{gender_table["U"]} gender undetermined cases, also with marginally different distributions between sides of the island (Chi-square, 
$X_{\Sexpr{gender_side_test$parameter}}=\Sexpr{signif(gender_side_test$statistic,2)}$,
$p=\Sexpr{signif(gender_side_test$p.value, 2)}$). 
...
\begin{table}[tbp]
\caption{\color{black}Fibropapillomatosis tumor presence in stranded turtles by side of \Hawaii\ island.
%Tumor distribution is unequal on the two sides of the island (Chi-square test, $X_2=198$, $p < 10^{-10}$).
}\label{tab:tumor_side}
<<tumor_side>>=
tumor_side_table <- BigIslandStrandings %>% count(Coast, Tumor) %>% 
  pivot_wider(names_from = Tumor, values_from = n)

tumor_side_test <- tumor_side_table %>% select(-Coast) %>% chisq.test()

tumor_side_table %>% relocate(Coast, Present=Y, None=N, "Not Recorded"=U) %>%
  bind_rows(., summarise(., across(where(is.numeric), sum), Coast="Total")) %>%
  knitr::kable("latex", booktabs=T, linesep=c("","\\addlinespace")) %>%
...
\Sexpr{sum(tumor_side_table$N)} records indicated the absence of FP tumors,
\Sexpr{sum(tumor_side_table$Y)} records presence of a tumor, and
\Sexpr{sum(tumor_side_table$U)} records are missing this observation. 
Note that the presence of a FP tumor does not necessarily mean that the primary cause of stranding was recorded as FP. 
Tumor presence/absence is significantly associated with side of the island, with turtles stranding in east \Hawaii\ more likely to have tumors than those in west \Hawaii\ (Chi-square test, $X_2=\Sexpr{number(tumor_side_test$statistic)}$, $p < 10^{-10}$).

\subsection{Stranding status}

\begin{table}[tbp]
\caption{\color{black}Survival status of stranded turtles by cause. Fibropapillomatosis is abbreviated FP.}\label{tab:cause_status}
<<cause_status>>=
cause_status_table <- BigIslandStrandings %>% count(Cause, Status=InitialStatus) %>%
  pivot_wider(names_from = Status, values_from = n, values_fill = 0) %>%
  rename(Alive=A, Dead=D, "Not Recorded" = U) 

cause_status_test <- cause_status_table %>% select(Alive, Dead) %>% chisq.test()

cause_status_table %>% bind_rows(., summarize(., across(where(is.numeric), sum), Cause="Total")) %>%
  knitr::kable("latex", booktabs=T, linesep=c(rep("",7), "\\addlinespace"))
@
\end{table}

\begin{table}[tbp]
\caption{\color{black}Survival status of stranded turtles by month.}\label{tab:month_status}
<<month_status>>=
BigIslandStrandings %>% mutate(Month=month(Date, label=TRUE, abbr=FALSE)) %>%
  count(Month, Status=InitialStatus) %>%
  pivot_wider(names_from = Status, values_from = n, values_fill = 0) %>%
  rename(Alive=A, Dead=D, "Not Recorded"=U) %>%
...
<<side_status, include=FALSE>>=
side_status_table <- BigIslandStrandings %>% count(Coast, InitialStatus) %>% 
  pivot_wider(names_from = InitialStatus, values_from = n, values_fill = 0) %>%
  column_to_rownames("Coast")
side_status_test <- side_status_table %>% select(-U) %>% chisq.test()
@


Of all the stranded turtles, 
\Sexpr{sum(side_status_table$A)} stranded alive, 
\Sexpr{sum(side_status_table$D)} stranded dead, and 
\Sexpr{sum(side_status_table$U)} turtles had no stranding status reported.
Stranding status was found to be significantly associated with cause 
(Chi-square test, $X_{\Sexpr{cause_status_test$parameter}} = \Sexpr{number(cause_status_test$statistic)}$,
$p < 10^{-10}$).
...
$X_{\Sexpr{side_status_test$parameter}} = \Sexpr{signif(side_status_test$statistic, 3)}$, 
$p = \Sexpr{side_status_test$p.value %>% signif(2)}$).
West \Hawaii\ had
\Sexpr{side_status_table["West","A"]} turtles strand alive and
\Sexpr{side_status_table["West","D"]} strand dead, 
while east \Hawaii\ had 
\Sexpr{side_status_table["East","A"]} turtles strand alive and
\Sexpr{side_status_table["East", "D"]} strand dead. 
...
\Sexpr{str_to_sentence(english(nrow(BigIslandStrandings)))} green turtles were recorded stranded on \Hawaii\ Island in the period 1983-2022, which represents an unknown fraction of total strandings on Hawaiian shores in that time. 
...
However, if a location is regularly accessed by more than a few people, a stranding is likely to be reported, and it is reasonable to believe that this will happen independently of the variables observed in these records.
...
Peak sea turtle stranding months during 2010-2019 in the Gulf of Mexico were also in the spring to summer (March to August) \citep{cook2021use, howell2021}.
...
Although slightly over half of the stranded green turtles in this study were listed with ``unknown'' cause of mortality, these turtles still provide valuable temporal, geographic, and biological data. 
We share this predicament of unknown cause with others studying sea turtle strandings. 
For example, 50\% of strandings in New Caledonia were unknown, defined as ``no necropsies were done and no apparent cause of death by external examination'' \citep{read2023twenty}. 
\citet{chaloupka2008cause} also had high rates of strandings with unknown causes.  
...
The relatively even distribution of turtles that stranded alive (\Sexpr{sum(side_status_table$A)}) versus dead \Sexpr{sum(side_status_table$D)} on \Hawaii\ Island in the present study is markedly different  from other research on O'ahu, Maui, and Kauai where approximately 75\% of green turtles stranded dead \citep{chaloupka2008cause} and on Taiwan where 80\% of stranded turtles were deceased \citep{cheng2019twenty}.
...
Study conception and design by Skylar Dentlinger, George Balazs, and Karla J. McDermid. 
Data compilation, initial analysis and first draft prepared by Skylar Dentlinger.
Follow-up analyses, modeling, and final manuscript and code preparation by Grady Weyenberg.
All authors made significant contributions to editing and revision of the manuscript, provided important intellectual content, and have read and approved the finalized version.
