...
A total of \Sexpr{Ntot} \xorig{754} stranded green turtles were reported from 1983--2022: \Sexpr{CoastCount$East} \orig{378} stranded on the east (windward) coast of \Hawaii\ Island and \Sexpr{CoastCount$West} \orig{376} on the west (leeward) coast.
...
A total of \Sexpr{Ntot} \xorig{754} green turtles stranded on \Hawaii\ Island from June 1983 to June 2022. Of those strandings, \Sexpr{CoastCount$West} (\Sexpr{percent(CoastCount$West/Ntot, 0.1, suffix="\\%")}) \orig{376 (49.9\%)} of those strandings were located on the leeward or west coast of \Hawaii\ Island, while \Sexpr{CoastCount$East} (\Sexpr{percent(CoastCount$East/Ntot, 0.1, suffix="\\%")}) \orig{378 (50.1\%)} were located on the windward side or east coast of \Hawaii\ Island (Figure \ref{fig:map}, Table \ref{tab:monthside}). 
...
The distribution of causes is significantly different between the East and West sides of the island (Chi-square test, $X_{\Sexpr{cause_side_test$parameter}}= \Sexpr{signif(cause_side_test$statistic, 3)}  \xorig{70}$, $p < 10^{-10}$), with the effect being driven most strongly by the FP and Miscellaneous categories (Table \ref{tab:cause}).
...
Study conception and design by Skylar Dentlinger and Karla J. McDermid. Initial analysis and first draft prepared by Skylar Dentlinger. Follow-up analyses and modelling by Grady Weyenberg. All authors contributed to editing and revision of the manuscript and have read and approved the finalized version.

This is an observational study compiled from publicly available records. No ethical approval is required.
